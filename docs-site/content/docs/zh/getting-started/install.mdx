...
[移动端同步指南 → Android](../guides/mobile-sync#android--其他客户端)。

<Callout type="info">
  **当前 Android 的限制。** Android 客户端走 **局域网伴侣** 模式：明文 HTTP + Basic Auth，
  没有 iroh P2P、没有 relay、没有 TLS、必须同一 Wi-Fi；它 **不是空间对端**，不会加入信任网格，
