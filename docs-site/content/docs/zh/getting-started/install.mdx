...
<Callout type="info">
  **当前 Android 的限制。** Android 客户端走 **局域网伴侣** 模式：明文 HTTP + Basic Auth， 没有 iroh
  P2P、没有 relay、没有 TLS、必须同一 Wi-Fi；它 **不是空间对端**，不会加入信任网格，
  只交换当下的剪贴板内容，不复制加密历史。Android 10+ 的后台剪贴板限制也可能导致 App 不在前台时
  收发延迟或被系统拦截。
</Callout>
...
App 走的是与桌面端的局域网同步（仅限同一 Wi-Fi）。连接流程详见
