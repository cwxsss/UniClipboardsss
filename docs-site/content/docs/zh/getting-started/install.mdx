...
<Callout type="warn">
  **遇到证书错误或 TestFlight 连不上 App Store Connect？先关代理梯子。**
  使用 Loon、Surge、Shadowrocket、Clash 等代理工具时，TestFlight 在打开
  邀请链接或点 **安装** 时可能出现 HTTPS / 证书报错，或一直转圈拉不到 App
  Store Connect。请 **临时关闭代理（含全局规则、TUN、HTTPS 解密 / MitM
  等）**，让 TestFlight 走直连；装完应用后再打开梯子即可。App 只跟你局域网
  内的桌面通信，不依赖代理。
</Callout>
