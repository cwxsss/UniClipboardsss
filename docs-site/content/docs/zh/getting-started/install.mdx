...
UniClipboard 注册全局快捷面板快捷键时，macOS 可能请求 **输入监控（Input Monitoring）** 权限。请在 **系统设置 → 隐私与安全性 → 输入监控** 中授权，然后完全退出并重新启动应用。

**辅助功能（Accessibility）** 是另一项独立的可选权限，只在启用快捷面板的双击修饰键触发方式时需要。UniClipboard 不会在后台主动请求；对应设置会显示一个按钮，由你明确打开 **隐私与安全性 → 辅助功能** 并授权，然后返回应用。剪贴板同步本身不依赖辅助功能权限。
...
发行包位于 `target/release/bundle/`。分支策略、commit 规范与 PR 流程见
...
—— 一个客户端同时覆盖 iOS 与 Android。因为桌面监听器讲的是
SyncClipboard 协议，其他任意 SyncClipboard 客户端也能接到同一个监听器。

直接到 [releases](https://github.com/UniClipboard/UniClip/releases/latest)
...
对于 GUI，首次启动应显示 **这是我的第一台设备** / **我已经在另一台设备上使用** 两个选项；如果已有空间，则直接进入仪表盘。
