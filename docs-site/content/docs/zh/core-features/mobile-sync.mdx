---
title: 移动端同步
description: 在桌面与 iPhone 或任意兼容 SyncClipboard 客户端之间同步剪贴 —— 默认走局域网，也可经自建 server 节点或 Tailscale 跨网络。
---
...
<Feature since="0.7.0" status="experimental">
  HTTP 伴侣模式 —— 默认走局域网，也可经自建 server 节点或 Tailscale 跨网络。iPhone 推荐使用
  **UniClipboard iOS App（TestFlight beta 公测中）**；也支持内置的 iOS 快捷指令或任意兼容
  SyncClipboard 协议的客户端。
</Feature>

移动端同步是一种 **HTTP 伴侣模式**（0.7 引入）：桌面守护进程暴露一个
兼容 SyncClipboard 的小型 HTTP 服务，已配对的手机用 HTTP Basic Auth
与之读写剪贴。

它能力上比桌面 ↔ 桌面同步窄一截：

- **不走 P2P。** 手机就是个普通 HTTP 客户端 —— 不打洞、不走中继。
  同一 LAN 下开箱即用；需要跨网络时，自建
  [无头 server 节点](../guides/self-host-server-node)（公网 HTTPS 端点）
  或走 Tailscale / VPN overlay（见 [下文](#跨网络同步搭配-tailscale)）。
- 监听器本身是明文 HTTP + Basic Auth —— TLS 由反向代理或 overlay 隧道
  提供，监听器自己不做。裸 LAN 下只在可信网络打开。
...
<Callout type="info">
  **当前 Android 限制：** 监听器是明文 HTTP + Basic Auth（TLS 由反向代理或 overlay 提供）、默认同
  LAN（跨网络可经 server 节点或 Tailscale）、移动端不进空间的信任网格，并且 Android 10+ 的后台剪贴板
  限制会让 App 不在前台时收发被延迟或拦截。
</Callout>
...
- server 节点：想要常在线、从任意网络都能跨网络访问（又不必给每台设备
  挂 VPN），可以跑一个无头 server 节点，用 TLS 反向代理把网关挡在前面 ——
  具体步骤见 [自建无头 server 节点](../guides/self-host-server-node)。
...
[故障排查 — 移动端同步（iOS 快捷指令）](../help/troubleshooting#移动端同步ios-快捷指令)。
