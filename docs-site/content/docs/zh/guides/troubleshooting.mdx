...
- **本机代理在转发 relay 流量。** 海外 relay + 本机开梯子时，默认规则常常把 relay 域名也走代理，导致带宽被代理出口压住、UDP/QUIC 兼容性变差。让 relay 域名走 DIRECT，详见 [自建 iroh relay — 本机有梯子时让 relay 走直连](./self-host-relay#本机有梯子时让-relay-走直连)。
