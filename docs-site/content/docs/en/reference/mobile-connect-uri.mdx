...
- An iOS shortcut, automation, or App that consumes the QR scanned from the
  UniClipboard desktop credential panel.
...
- The credential panel warns the user ("save now — won't be shown again") and
  the password is never persisted on the desktop side beyond the panel's
  display lifecycle.
...
- MUST NOT include the URI in analytics events, crash reports, or error
  attachments.
- MUST NOT persist the URI to disk after the credential panel is dismissed — store
