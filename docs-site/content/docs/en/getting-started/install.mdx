...
path; see the [Mobile sync guide → Android](../guides/mobile-sync#android--other-clients).

<Callout type="info">
  **Android limitations today.** The Android client runs as a **LAN companion** over plain HTTP +
  Basic Auth — no iroh P2P, no relay, no TLS, same-Wi-Fi only — and it is not a peer of your
  space's trust mesh (it only exchanges the current clipboard, not the encrypted history). Android's
...
the [Mobile sync guide](../guides/mobile-sync) for the full
pairing flow.
