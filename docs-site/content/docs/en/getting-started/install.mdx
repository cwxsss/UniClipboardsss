...
<Callout type="info">
  **Android limitations today.** The Android client runs as an **HTTP companion** over plain HTTP +
  ...
  space's trust mesh (it only exchanges the current clipboard, not the encrypted history). It reaches
  the desktop on the LAN by default, or across networks via a
  [headless server node](../guides/self-host-server-node) or Tailscale. Android's
