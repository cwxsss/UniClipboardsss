...
If you enable the Quick Panel's modifier double-tap trigger, the GUI process samples the local
keyboard state while that trigger is active. It reduces each sample immediately to two booleans:
whether the selected modifier is down and whether any other key is down. Raw keys are not logged,
persisted, indexed, included in diagnostics, or sent to another device or service. Disabling the
trigger stops the sampling worker. On macOS this capability requires the separate Accessibility
