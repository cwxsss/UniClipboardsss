...
fresh one-time password and atomically invalidates the previous
Argon2id hash; changing the username reissues the password too. The old
credentials stop working immediately, so update the phone (UniClipboard
iOS App, Shortcut, or other client) with the new credentials.
