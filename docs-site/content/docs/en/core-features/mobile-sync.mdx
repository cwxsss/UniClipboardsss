...
- **Big QR (top, always on)** — encodes the base URL and credentials
  as a single `uniclipboard://connect` URI. Scan it from the
  **UniClipboard iOS App** (TestFlight public beta — recommended;
  see [Pair an iPhone](#pair-an-iphone) below), the
  ...
  switch the advertised IP (LAN candidates, plus Tailscale CGNAT
  if you have it) without re-opening the Configure modal.
...
<Callout type="info">
  **Android limitations today.** No TLS at the listener (plain HTTP + Basic Auth — TLS comes from a
  reverse proxy or overlay), same-LAN by default (cross-network via a server node or Tailscale), not a
  peer of your space's trust mesh, and Android 10+ background-clipboard restrictions can delay or
...
| Revoke a device      | Click device card → dialog → **Revoke**          | `uniclip mobile-sync devices revoke <device-id>`       |
