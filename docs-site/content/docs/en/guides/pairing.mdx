...
The first device runs `init` (CLI) or **This is my first device** (GUI).
That step:
...
| Transport      | iroh QUIC, P2P first, encrypted relay fallback | Plain HTTP + Basic Auth — **no QUIC, no relay** (TLS comes from a reverse proxy)                         |
| Cross-network  | Yes (hole-punching, relay)                     | **Yes, via a server node or Tailscale** — not P2P; the phone reaches an HTTPS gateway or overlay address |
| End-to-end key | XChaCha20-Poly1305 under the space MasterKey   | TLS-less HTTP; payload hashed and authenticated only                                                     |

The phone is always just a SyncClipboard HTTP client — it never becomes
an iroh peer. "Cross-network," then, means reaching the gateway from
...
2. The daemon mints a one-time username and password and shows them
   ...
   - **Android / other clients** — copy the base URL, username, and
     ...
     install the **UniClipboard Android client** at
...
- **No iroh / no relay.** The mobile listener is a separate HTTP
  ...
  no QUIC. Off-LAN clients can't reach the desktop _directly_ — front
  the gateway with a [headless server node](./self-host-server-node)
  (public HTTPS) or a Tailscale / VPN overlay to reach it from another
  network.
...
- **No per-direction / per-content-type gates yet.** The
  [per-peer toggles](../core-features/devices#content-type-allowlist) on the
...
- [Sync content](../core-features/sync) — the payload types that flow over this
  pipeline.
- [Quick start](../getting-started/quick-start) — the user-facing
  flow that exercises pairing end-to-end.
- [CLI reference](../cli/getting-started) — the headless equivalents of
