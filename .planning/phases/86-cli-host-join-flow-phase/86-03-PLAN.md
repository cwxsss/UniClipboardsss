...
must_haves:
  truths:
    - 'HostCliPhase enum has 6 variants with session_id in NeedDecision and NeedVerification'
    - 'JoinCliPhase enum has 8 variants with session_id in NeedPeerConfirmation'
    - 'derive_host_phase() and derive_join_phase() are pure functions'
    - 'last_submitted_* fields are used for idempotent deduplication only'
  artifacts:
