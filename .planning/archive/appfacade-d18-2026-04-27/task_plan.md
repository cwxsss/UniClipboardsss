...
### Phase D9 · daemon 下一块收口 — complete (2026-04-27, 由 D10~D18 收尾)
...
**收尾说明(2026-04-27)**:
- 后续 D10~D18 已分别完成 clipboard history、search/restore、auto-unlock、daemon/tauri/cli runtime 去旧入口、uc-app 退役,以及 D18 的 AppFacade 唯一入口补漏。
- D18 验证后,daemon 主路径已不再保留 `SpaceAccessFacade` 旁路;D9 作为母任务关闭。
...
### Phase D13 · daemon 去 CoreRuntime 化 — complete (2026-04-26)

**触发与原则纠正**:

D12 完成后,daemon 已脱离 `uc-app::usecases`,但仍在 `app.rs` / `entrypoint.rs` / `api/server.rs` 持有 `Arc<CoreRuntime>`,通过 `runtime.wiring_deps().<port>` 直接掏 ports 拼 facade deps。这是封装漏洞——daemon 在"穿透 application 摸 ports"。

用户拨正方向:
1. **`AppFacade` 是唯一对外关卡** — 外部调用 100% 走 `AppFacade`,看不到 ports / runtime / deps / coordinator / mode。
2. **uc-application 只装"业务"** — `ClipboardIntegrationMode`(部署模式)、`TaskRegistry`(tokio 任务工具)、`AppPaths`(进程路径)、`AppDeps`(ports 包) 这些**外部环境/进程基础设施**不属于 application,application 不该知道也不该持有。
3. **`CoreRuntime` 这个抽象应该在外部消失** — 不是搬到 application,而是让 daemon/Tauri/CLI 直接持 `Arc<AppFacade>` + 各自需要的本地零碎件,不再用 `CoreRuntime` 把它们打包当成"应用 runtime"。

**长期路线图**(uc-app 终态):
```
D13  daemon 去 CoreRuntime
D14  uc-tauri 去 AppRuntime
D15  uc-cli 去 CoreUseCases
       ↓ 三个调用方完成后
D16  清理 uc-app 内部:usecases shim 删除、AppDeps 内部化进 application、TaskRegistry 搬 uc-bootstrap、CoreRuntime 类型删除
D17  workspace 移除 uc-app crate
```

注意:**"删 uc-app crate"是终末副产品,不是阶段性目标**。每一步的目标是"让一个调用方不再穿透 application 摸 ports"。

**D13 目标**(本切片):

让 daemon 整个 crate 不再依赖 `uc_app::runtime::CoreRuntime`:
- `DaemonApp` 持 `Arc<AppFacade>` + 几个独立字段(`Arc<TaskRegistry>` / `AppPaths` / `Arc<dyn LifecycleStatusPort>` / `event_emitter` cell / `ClipboardIntegrationMode`),不再持 `Arc<CoreRuntime>`。
- `AppFacade` 在 `entrypoint.rs` 装配,装好后 `Arc<AppFacade>` 传给 `DaemonApp::new_with_deferred`。
- `DaemonApp::run` 内 14 处 `self.runtime.wiring_deps().<port>` 全部消失(因为 facade 已经在 entrypoint 装好了)。
- `recover_encryption_session(&CoreRuntime, bool)` → `recover_encryption_session(&AppFacade, bool)`。
- `DaemonApiState::runtime: Option<Arc<CoreRuntime>>` 字段去除,handler 全部走 `state.app_facade`。
- `cargo check --workspace` 通过(uc-cli `RosterError::Unavailable` 历史欠账除外)。
- `grep -rn "CoreRuntime\|uc_app::runtime" uc-daemon/` 返回 0 hit。

**子任务**(均建 TaskCreate):
- T1 — daemon CoreRuntime 调用清单 + AppFacade 路径映射
- T2 — facade 缺口补齐(若有)
- T3 — `entrypoint.rs` 内装 `AppFacade`
- T4 — `DaemonApp` 去 `CoreRuntime` 字段
- T5 — `recover_encryption_session` 签名改造
- T6 — `DaemonApiState::runtime` 字段下线
- T7 — `cargo check` 全 workspace + 修崩
- T8 — commit

**预期保留在 uc-app**(D13 不动):
- `CoreRuntime` / `AppDeps` / `TaskRegistry` 类型本身仍住在 uc-app(uc-bootstrap 还在用)
- uc-tauri / uc-cli 仍引用 uc_app::*(D14/D15 处理)

**风险与规避**:
- daemon 装 facade 时需要 14+ 个 ports,如果在 entrypoint 里装,要把 wiring 流程提前——但 entrypoint 本来就先调 `wire_dependencies` 再造 `CoreRuntime`,只需把 `AppFacade::new(...)` 那段从 `DaemonApp::run` 里搬过来,前置依赖完整。
- `set_event_emitter` 调用现在通过 runtime;改造后 daemon 直接持 emitter cell,bootstrap 期 swap 直接对 cell 操作。
- HTTP handler(api/search.rs)的 `runtime.is_encryption_ready()` 改 `state.app_facade.encryption.state().session_ready`,语义等价。

**实际改动**:

`uc-daemon/src/app.rs`:
- 删除 `use uc_app::runtime::CoreRuntime;`、`AppLifecycleStatusGateway` 包装类、`lifecycle_state_to_view` / `lifecycle_state_from_view` 转换函数(`LifecycleStatusPort` 实际就是 `LifecycleStatusGateway` 的别名,wrapper 完全冗余)。
- `DaemonApp` 字段:删 `runtime/space_setup_facade/member_roster_facade/search_coordinator`;加 `app_facade: Arc<AppFacade>` / `storage_paths: AppPaths` / `event_emitter_cell: Arc<RwLock<Arc<dyn HostEventEmitterPort>>>`。`local_device_id` 字段保留(sponsor view,daemon 自身的运行时配置)。
- `recover_encryption_session(&AppFacade, bool)`:内部用 `facade.encryption.state().initialized` 替代 `runtime.wiring_deps().setup_status.get_status()`,用 `facade.encryption.unlock()` 替代旧 use case path。
- `run()` 方法删除整段 `AppFacade::new(AppFacadeParts {..})` (~110 行,搬到 entrypoint),`storage_paths` 直接读 `self.storage_paths`,emitter 写入直接对 `self.event_emitter_cell` 操作。
- 行数从 601 → ~390(删了 wrapper 和 facade 装配段)。

`uc-daemon/src/entrypoint.rs`:
- 在 `DaemonApp::new_with_deferred(...)` 之前插入 ~80 行 `AppFacade::new(AppFacadeParts {..})`,从 `runtime.wiring_deps()` 拼 deps(entrypoint 是 daemon 端 composition root,认 CoreRuntime 不违反"AppFacade 是唯一关卡"原则——关卡是给业务代码,不是给 bootstrap)。
- `recover_encryption_session(&unlock_runtime, ...)` → `recover_encryption_session(&unlock_app_facade, ...)`。
- background unlock task 中读 settings 仍走 `unlock_runtime.wiring_deps().settings.load()`(entrypoint 内部行为,合规)。
- `build_non_gui_runtime_with_emitter` 调用改为传 `emitter_cell.clone()`,daemon 也持有 cell 副本。

`uc-daemon/src/api/server.rs`:
- 删除 `use uc_app::runtime::CoreRuntime;`、`runtime: Option<Arc<CoreRuntime>>` 字段、`runtime_or_error()` 方法、`DaemonApiState::new` 第三个参数。

`uc-daemon/src/api/search.rs`:
- `let runtime = state.runtime_or_error()?; if !runtime.is_encryption_ready().await {..}` → `let app_facade = state.app_facade_or_error()?; let encryption_state = app_facade.encryption.state().await?; if !encryption_state.session_ready {..}`。

`uc-daemon/src/security/connect.rs`:
- 注释里 `CoreRuntime` 改 `AppFacade::encryption`(死注释清理)。

`uc-daemon/Cargo.toml`:
- 删除 `uc-app = { path = "../uc-app" }` 依赖。**daemon crate 现在完全不依赖 uc-app**。

**验证**:
- `cargo check -p uc-daemon`:✅ passed(`uc-app` 依赖移除后仍编译)
- `cargo test -p uc-daemon --lib`:✅ 25 passed
- `cargo check --workspace`:✅ uc-app/uc-daemon/uc-application/uc-bootstrap/uc-daemon-local/uc-daemon-client/uc-tauri 全部通过;uc-cli `RosterError::Unavailable` non-exhaustive 是 commit 4aba60cc 引入历史欠账,与本次无关。
- `grep -rn "CoreRuntime\|uc_app::runtime\|use uc_app" uc-daemon/`:✅ 0 hit。

**残留与下一步**:
- D14 — `uc-tauri::bootstrap::runtime` 里的 `AppRuntime` 仍包 `Arc<CoreRuntime>` + `CoreUseCases`,Tauri command 走 facade。
...
### Phase D15 · uc-cli setup 命令去 CoreUseCases 化 — complete (2026-04-26)

**触发**:

D14 完成后,uc-cli 的 9 个命令(join/members/invite/send/blob/init/watch/space_status)已经走 `uc_application::facade::*`。**唯一遗留**:`commands/setup.rs::run_new_space()`(L94-158)还在用 `CoreUseCases::new(&runtime).initialize_encryption().execute(passphrase)` + `uc.setup_status().mark_complete()`,以及 `commands/space_status.rs::run()` 用 `runtime.is_encryption_ready()` + `runtime.has_completed_setup()` 走 CoreRuntime。

**预调查**:
- F-143: `EncryptionFacade` 当前没有 `initialize(passphrase)` 方法,只有 `state / unlock / lock / verify_keychain_access`
- F-144: `SetupStatusFacade` 已存在(`is_complete / mark_complete`),但**未聚合进** `AppFacadeParts`
- F-145: `InitializeEncryption` use case 是非常薄的 wrapper —— 仅做 model::Passphrase → domain::Passphrase 桥接 + `space_access.initialize(&space_id, &passphrase)`,逻辑可直接内联进 facade
- F-146: CLI 端"创建空间"语义包含两步:`space_access.initialize` + `setup_status.set_status(has_completed=true)`,这两步语义上是同一原子动作,facade 应合并

**D15 目标**:

让 `uc-cli` 不再 `use uc_app::usecases::CoreUseCases;`,setup/status 命令通过 `Arc<AppFacade>` 完成所有动作:

- `EncryptionFacade::initialize(passphrase: model::Passphrase) -> Result<(), EncryptionFacadeError>` 新增 —— 内部完成 space_access.initialize + setup_status mark complete 两步
- `uc_bootstrap::build_cli_app_facade(profile)` 新增 —— 一行装出 `Arc<AppFacade>` 给 CLI 用(类比 build_cli_runtime)
- `commands/setup.rs::run_new_space()`:`CoreUseCases::new(&runtime).initialize_encryption().execute(...)` + `uc.setup_status().mark_complete()` → `app_facade.encryption.initialize(passphrase)` 一步
- `commands/setup.rs`:`runtime.has_completed_setup()` → `app_facade.encryption.state().initialized`
- `commands/space_status.rs::run()`:`runtime.is_encryption_ready() / has_completed_setup()` → `app_facade.encryption.state().{session_ready, initialized}`
- 删 `commands/setup.rs` 顶 `use uc_app::usecases::CoreUseCases;`
- 验证:`grep -rn "CoreUseCases\|uc_app::" uc-cli/src/` 仅余 facade-allowed 引用(应 = 0 hit)
- `uc-cli/Cargo.toml` 仍保留 uc-app 依赖(`build_cli_runtime` 还会被部分调用方持续引用,保留至 D16 一并清理)

**子任务**:
- T1 — `EncryptionFacade::initialize(passphrase)` 新方法 + 错误映射(`AlreadyInitialized` 透传 + `SetupStatus` 新错误变体)
- T2 — `EncryptionFacade::initialize` 单元测试(覆盖 happy path / already initialized / setup status fail)
- T3 — `uc_bootstrap::build_cli_app_facade(profile) -> anyhow::Result<Arc<AppFacade>>` 助手函数
- T4 — `setup.rs::run_new_space` 改造
- T5 — `space_status.rs::run` 改造
- T6 — 删 `setup.rs` 死 import
- T7 — `cargo check --workspace --all-targets` + `cargo test --workspace --lib`
- T8 — commit

**风险与规避**:
- `EncryptionFacade::initialize` 合并 setup_status mark complete 后,需要保证幂等 —— 若 space_access.initialize 失败需要 abort,不该 mark complete;已在 T1 中明确顺序(先 init 后 mark)
- `build_cli_app_facade` 装配代码与 daemon entrypoint 装配代码会有大量重复,但暂不抽公用 helper —— 抽公用 helper 是 D16 范围(那时 ClipboardWriteCoordinator + AppDeps 也要内部化);现在让 CLI / daemon / Tauri 各装一份,清晰可读

**残留**:
- D16:`InitializeEncryption` use case + `CoreUseCases::initialize_encryption / setup_status` 两个工厂方法变 dead code,届时可删
- D16:`build_cli_runtime` 会被 D17 删除(随 CoreRuntime 类型一起)

**实际改动**:

`uc-application/src/facade/encryption/mod.rs`:
- `EncryptionFacadeError` 加 `AlreadyInitialized` 变体
- `EncryptionFacade::initialize(passphrase: model::Passphrase) -> Result<()>` 新方法 —— 内部调 `space_access.initialize(&space_id, &domain_passphrase)` + `setup_status.set_status(has_completed=true)`,两步原子,第一步失败立即返回不写 status
- 测试加 `initialize_creates_space_and_marks_setup_complete` / `initialize_maps_already_initialized_error` 两个覆盖

`uc-application/src/facade/app_facade.rs`:
- `clipboard_restore` 字段从 `Arc<ClipboardRestoreFacade>` → `Option<Arc<ClipboardRestoreFacade>>`(与 `space_setup` / `member_roster` 一致),让 CLI 端可以 `None` 跳过该 facade

`uc-bootstrap/src/non_gui_runtime.rs`:
- 加 `build_cli_app_facade(profile) -> anyhow::Result<Arc<AppFacade>>` 助手 —— 装 8 个 facade(encryption / settings / device / clipboard_history / search / lifecycle / storage / resource),`space_setup` / `member_roster` / `clipboard_restore` 全 None
- import 列表扩到包含所有 facade 类型

`uc-bootstrap/src/lib.rs`:
- 顶层 `pub use` 加 `build_cli_app_facade`

`uc-cli/src/commands/setup.rs`:
- 删 `use uc_app::usecases::CoreUseCases;`
- `run_new_space()`:`build_cli_runtime` → `build_cli_app_facade`;`runtime.has_completed_setup()` → `app_facade.encryption.state().initialized`;两步 `uc.initialize_encryption().execute(...) + uc.setup_status().mark_complete()` 合并为单步 `app_facade.encryption.initialize(passphrase)`(facade 内部已合并)

`uc-cli/src/commands/space_status.rs`:
- `build_cli_runtime` → `build_cli_app_facade`
- `runtime.is_encryption_ready() / has_completed_setup()` → `app_facade.encryption.state().{session_ready, initialized}`

`uc-daemon/src/api/routes.rs`:
- `restore_clipboard_entry_handler` 适配 `app.clipboard_restore: Option<...>`,unavailable 时返回 internal error(daemon 实际总是 Some,这是防御性代码)

`uc-daemon/src/entrypoint.rs` / `uc-tauri/src/bootstrap/runtime.rs`:
- `clipboard_restore: Arc::new(...)` → `clipboard_restore: Some(Arc::new(...))`(适配字段类型变化)

**验证**:
- `cargo check --workspace --all-targets` ✅
- `cargo test --workspace --lib` ✅ all passing(uc-application 25→27,新加 2 个 initialize 测试)
- `grep -rn "CoreUseCases\|uc_app::usecases\|use uc_app" uc-cli/src/` = **0 hit** —— uc-cli 的 setup 命令完全脱离 uc-app::usecases

**残留与下一步**:
- `uc-cli/Cargo.toml` 仍依赖 uc-app(`build_cli_runtime` 仍存在,但 D14/D15 后无 caller,可在 D16 一并清理)
- `InitializeEncryption` use case + `CoreUseCases::initialize_encryption` / `CoreUseCases::setup_status` 工厂方法变 dead code,D16 删
- D16:清理 uc-app 内部 —— `AppDeps` 内部化进 application、`TaskRegistry` 搬 uc-bootstrap、`CoreRuntime` 类型删除、`ClipboardWriteCoordinator` shim 删除
- D17:workspace 移除 uc-app crate


---

### Phase D16-1 · uc-app dead use cases 清理 + Cargo 依赖收缩 — complete (2026-04-26)

**触发**:

D14/D15 完成后,uc-app 内部留下大量 dead code:`CoreUseCases` 类型 + 13 个 use cases 的工厂方法已经无 caller。**完全没人用**的 uc-app 路径(grep `uc_app::usecases::*` 在 uc-cli / uc-bootstrap / uc-tauri / uc-daemon / uc-application 全 0 hit):

- `uc_app::usecases::CoreUseCases`(D14/D15 后)
- `uc_app::usecases::InitializeEncryption`(D15 后)
- `uc_app::usecases::VerifyKeychainAccess`(D11/D12 已迁 facade)
- `uc_app::usecases::GetSettings` / `UpdateSettings` / `DeleteClipboardEntry` / `ClearClipboardHistory` / `IndexClipboardEntry` / `SearchClipboardEntries` / `RebuildSearchIndex` / `GetMemberSyncPreferences` / `UpdateMemberSyncPreferences` / `GetLocalDeviceInfo` / `ListClipboardEntryProjections` / `OutboundSyncPlanner`
- `uc_app::usecases::clipboard::ClipboardUseCases`(及 12 个 sub usecases)
- `uc_app::usecases::file_sync::CopyFileToClipboard`
- `uc_app::usecases::internal::*`(2 个)
- `uc_app::usecases::membership::*` / `pairing::*` / `search::*`(3 个) / `storage::*`(2 个) / `sync_planner::*`
- `uc_app::change_passphrase::*`
- `uc_app::App` struct(无 caller)

并且:
- `uc-cli/Cargo.toml` 仍依赖 uc-app —— 但 grep `\buc_app::` 在 uc-cli/src/ 是 0 hit
- `uc-daemon-local/Cargo.toml` 仍依赖 uc-app —— 0 hit
- `src-tauri/Cargo.toml`(uniclipboard binary) 仍依赖 uc-app —— 0 hit

**仅剩有 caller 的 uc-app 类型**:
- `uc_app::AppDeps` / `ClipboardPorts` / `DevicePorts` / `SecurityPorts` / `StoragePorts` / `SystemPorts` / `deps::SearchPorts` —— 留 D16-2 搬到 uc-application
- `uc_app::task_registry::TaskRegistry` —— 留 D16-2 搬到 uc-bootstrap
- `uc_app::runtime::CoreRuntime` —— 留 D16-2 删除
- `uc_app::usecases::ClipboardWriteCoordinator` —— 留 D16-2,改 caller 用 `uc_application::clipboard_write::ClipboardWriteCoordinator` 真身
- `uc_app::usecases::file_sync::CleanupExpiredFilesUseCase` —— 留 D16-2 搬到 uc-application 或继续保留
- `uc_app::usecases::LifecycleStatusPort` 等 lifecycle re-export shim —— `runtime.rs` 用,留 D16-2 一并清

**D16-1 目标**(本切片):

清掉所有"零 caller"的 uc-app dead code,卸载 uc-cli / uc-daemon-local / uniclipboard binary 的 uc-app Cargo 依赖。**不**搬迁仍在用的类型(留 D16-2)。

具体动作:
1. 删除 uc-app 内部 dead use case 文件:
   - `usecases/initialize_encryption.rs` / `verify_keychain_access.rs` / `change_passphrase.rs` / `delete_clipboard_entry.rs` / `get_settings.rs` / `update_settings.rs`
   - `usecases/clipboard/` 下的 11 个 dead use cases(保留 `clipboard_write_coordinator.rs` + `integration_mode.rs`)
   - `usecases/file_sync/copy_file_to_clipboard.rs`(保留 `cleanup.rs`)
   - `usecases/internal/`(整个目录)
   - `usecases/membership/`(整个) / `pairing/`(整个) / `search/`(整个) / `storage/`(整个) / `sync_planner.rs`
2. 重写 `usecases/mod.rs`:删 CoreUseCases 类型 + 收窄 pub use 到 ClipboardWriteCoordinator + CleanupExpiredFilesUseCase + LifecycleStatusPort 这三个仍在用的
3. 修 `lib.rs`:删 `App` struct + `pub use usecases::CoreUseCases;`
4. 卸载 Cargo 依赖:`uc-cli` / `uc-daemon-local` / `src-tauri/Cargo.toml`(uniclipboard binary)
5. 验证:`cargo check --workspace --all-targets` 全过 + `cargo test --workspace --lib` 全过

**预期影响**:
- uc-app crate src/ 行数从 ~2k → ~500(-75%)
- 4 个 crate 的 Cargo.toml 收缩(uc-app 依赖只剩 uc-bootstrap / uc-tauri 两处)

**风险与规避**:
- 删除的 use case 中可能有 uc-app 内部相互引用 —— 由 cargo check 自动暴露,逐个修
- runtime.rs 用 `crate::usecases::LifecycleStatusPort` 和 `crate::usecases::ClipboardWriteCoordinator`,这两个保留的 mod 路径必须可用
- 测试文件可能引用 dead use cases —— 一并删除测试

**残留**:
- D16-2:`AppDeps` 内部化进 uc-application、`TaskRegistry` 搬 uc-bootstrap、`CoreRuntime` 删除、`ClipboardWriteCoordinator` shim 删除
- D17:workspace 移除 uc-app crate
...
文件删除(uc-app/src/):
- `usecases/initialize_encryption.rs` / `verify_keychain_access.rs` / `change_passphrase.rs` / `delete_clipboard_entry.rs` / `get_settings.rs` / `update_settings.rs`
- `usecases/clipboard/`:`clear_history.rs` / `get_entry_detail.rs` / `get_entry_resource.rs` / `resolve_blob_resource.rs` / `resolve_thumbnail_resource.rs` / `touch_clipboard_entry.rs` / `toggle_favorite_clipboard_entry.rs` / `restore_clipboard_selection.rs` / `list_entry_projections/`(整个目录)
- `usecases/file_sync/copy_file_to_clipboard.rs`
- `usecases/internal/`(整个目录:capture_clipboard + resolve_clipboard_selection)
- `usecases/membership/`(整个) / `pairing/`(整个) / `search/`(整个) / `storage/`(整个) / `sync_planner/`(整个)

文件重写(uc-app/src/):
- `usecases/mod.rs`:从 ~280 行 CoreUseCases 巨型 impl 缩到 ~30 行,只 `pub mod app_lifecycle/clipboard/file_sync` + `pub use` 仍在用的 3 个类型
- `usecases/clipboard/mod.rs`:删 `ClipboardUseCases` / `ClipboardStats` / `compute_clipboard_stats` 等 dead helper,只剩 2 个 mod 声明 + 1 个 re-export
- `usecases/file_sync/mod.rs`:从 7 行 `pub use` 4 个类型 → 3 行只 re-export `CleanupExpiredFilesUseCase`
- `lib.rs`:删 `App` struct + `pub use usecases::CoreUseCases;`,顶部 docstring 重写说明残留状态

Cargo.toml 卸载 uc-app 依赖:
- `uc-cli/Cargo.toml`(无 `uc_app::` 引用)
- `uc-daemon-local/Cargo.toml`(无 `uc_app::` 引用)
- `src-tauri/Cargo.toml`(uniclipboard binary 也无 `uc_app::` 引用)

**验证**:
- `cargo check --workspace --all-targets` ✅(仅 2 个无关 dead_code warning,与 D16-1 无关)
- `cargo test --workspace --lib` ✅ all passing(uc-app 41 / uc-application 25 / uc-bootstrap 13 / uc-platform 6 / uc-infra 124 / uc-daemon 25 / uc-tauri 0,0 failures)
- `grep -rE "(^|[^a-z])uc_app(::|[^a-z_])"` 在 uc-cli/src/ + uc-daemon-local/src/ + src-tauri/src/main.rs = **0 hit**

**意义**:
- uc-app crate src/ 体积大幅收缩 —— 删除 ~30 个 dead use case 模块
- 依赖 uc-app 的 crate 从 5 个降到 2 个(只剩 uc-bootstrap / uc-tauri,这两个 D16-2 处理)
- uc-cli / uc-daemon-local / uniclipboard binary 完全脱离 uc-app
...
### Phase D18 · 收紧 AppFacade 唯一入口(legacy `SpaceAccessFacade` + 子 facade 直注入清理) — complete (2026-04-27)
...
#### D18.3 · 实施方案 D 彻底清理 — complete (2026-04-27, 分多个 atomic commit)
...
- ✅ 验证:`npm run build` / `npm run test -- --run`(项目无 `npm run check` 脚本)
...
##### D18.3.g · 全量验证 — complete (2026-04-27)
- ✅ `cargo check --workspace --all-targets` 通过(2 条既有 dead_code warning)
- ✅ `npm run build` 通过(仅 Vite chunk size warning)
- ✅ `npm run test -- --run` 通过:58 files / 319 tests
- ✅ grep 复核:`rg "SpaceAccessFacade|SpaceAccessOrchestrator|space_access_facade|RealtimeTopic::SpaceAccess|ws_topic::SPACE_ACCESS|space-access" src-tauri src` = 0 hit
- ⏭️ 按用户 2026-04-27 指示跳过:`cargo clippy --workspace -- -D warnings` / `cargo test --workspace --lib`

#### D18.4 · 决策:`require_facade` 局部解构是否保留 — complete (2026-04-27)
- ✅ 评估 `api/v2/setup.rs:43` / `api/clipboard.rs:47`
- ✅ 决策:保留。它们只是从 `state.app_facade_or_error()?` 取出子 facade 的 handler 局部别名,没有持有独立入口,也没有绕过 AppFacade 装配边界
- ✅ findings.md 记录决策,本 phase 不改 handler 代码
...
- D18.3 `complete` ✅ (2026-04-27, a~g 完成)
- D18.4 `complete` ✅ (2026-04-27)
