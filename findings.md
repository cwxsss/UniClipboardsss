...
### F-137 · clipboard watcher 的 live search index 可下沉到 application

**发现时间**:2026-04-26

**背景**:`DaemonClipboardChangeHandler` capture 成功后,在 worker 内直接读取 entry、执行 representation policy、构建 search pipeline、派生 search key,再调用 index use case。这让 watcher 直接拥有 search indexing 规则。

**处理结果**:
- 新增 `ClipboardLiveIndexFacade` / `ClipboardLiveIndexer`。
- application 层接管 capture 后 live index 的 entry 读取、selection、projection、key、pipeline 和 index 写入。
- daemon watcher 改为只调用 `ClipboardLiveIndexFacade::index_capture(...)`。
- daemon watcher 不再直接引用 `CoreUseCases`、`SearchProjectionBuilder`、search key / pipeline / index 相关对象。

**边界判断**:
- watcher 内 outbound planning 和 blob refs dispatch 仍待收口。
- `entrypoint.rs` 仍负责从 runtime wiring deps 组装 live index deps,归入 composition root 收口。
...
### F-136 · clipboard watcher 的 capture 落库可先通过 application facade

**发现时间**:2026-04-26

**背景**:`DaemonClipboardChangeHandler` 原来在 worker 内直接从 runtime wiring deps 构造 `CaptureClipboardUseCase`,并直接调用 `execute_with_origin`。后台 watcher 是外部入口,不应自己组装 use case。

**处理结果**:
- 新增 `uc-application::facade::clipboard_capture::ClipboardCaptureFacade`。
- application 层暴露 `CapturedClipboardEntryView`,对外只返回稳定字符串 entry id。
- daemon watcher 改为调用 `ClipboardCaptureFacade::capture(...)`。
- daemon watcher 不再 import / 构造 `CaptureClipboardUseCase`。

**边界判断**:
- watcher 内的 search live index、outbound planning、blob refs dispatch 仍在 daemon,继续留在 clipboard workers todo。
- `entrypoint.rs` 仍负责构造 capture use case 并包成 facade,归入 composition root 收口。
...
### F-135 · inbound clipboard worker 可先通过 application facade 调用入站应用

**发现时间**:2026-04-26

**背景**:`InboundClipboardSyncWorker` 原来直接持有 `ApplyInboundClipboardUseCase`,并直接处理 `ApplyOutcome` / `EntryId`。它是 daemon 后台入口的一部分,不应该直接依赖 use case 和 core id 类型。

**处理结果**:
- 新增 `uc-application::facade::clipboard_inbound::InboundClipboardFacade`。
- application 层暴露 `InboundClipboardNoticeInput` / `InboundClipboardApplyOutcome` 等 worker-facing 模型。
- daemon inbound worker 改为调用 `InboundClipboardFacade::apply_notice(...)`。
- daemon inbound worker 不再 import `ApplyInboundClipboardUseCase`、`ApplyInboundInput`、`ApplyOutcome` 或 core `EntryId`。

**边界判断**:
- `entrypoint.rs` 仍负责构造 `ApplyInboundClipboardUseCase` 并包成 facade,属于 composition root 后续收口。
- outbound clipboard watcher 仍有更大的旧依赖,继续留在 clipboard workers todo。
...
### F-134 · search coordinator 本体可下沉到 application

**发现时间**:2026-04-26

**背景**:`uc-daemon/src/search/coordinator.rs` 原来直接持有 core runtime、读取 index meta、判断版本、拉取剪贴板数据、构建索引输入并触发 rebuild。HTTP/WS handler 已经走 `AppFacade`,但后台 search coordinator 仍是 daemon 拥有业务规则。

**处理结果**:
- 将 search coordinator 的状态、启动检查、重建串行化、进度事件迁入 `uc-application`。
- application coordinator 直接通过注入的 ports 工作,不再依赖 daemon 类型。
- daemon search 模块只保留一个服务包装,负责启动 application coordinator 并把 application 事件转成 WS 消息。
- `SearchFacade.status()` 改为从 application coordinator 读取完整状态视图。

**边界判断**:
- daemon `entrypoint.rs` 仍负责从 runtime 取 ports 组装 coordinator deps,归入 composition root 收口 todo。
- daemon search HTTP/WS 调用面、projection、coordinator 均已不再直接拥有 core/infra 搜索规则。
...
### F-133 · search projection 规则可先迁入 application

**发现时间**:2026-04-26

**背景**:`uc-daemon/src/search/projection.rs` 维护了从剪贴板数据构造搜索 pipeline 输入的规则,同时被 search coordinator 和 clipboard watcher 使用。该规则属于 application/search 边界,不应由 daemon 拥有。

**处理结果**:
- 将 `SearchProjectionBuilder` 移到 `uc-application/src/facade/search/projection.rs`。
- `uc-application::facade` 统一导出 `SearchProjectionBuilder`。
- daemon search coordinator 和 clipboard watcher 改为从 application 调用 projection builder。
- 删除 daemon search projection 模块。

**边界判断**:
- search coordinator 仍在 daemon 下,仍直接知道 `CoreRuntime` 和 index 版本常量。这是 search todo 的剩余部分,后续继续处理。
...
### F-132 · presence monitor 的 WS 推送可经 AppFacade 订阅和取快照

**发现时间**:2026-04-26

**背景**:`PresenceMonitor` 原来直接订阅 `PresencePort`,并通过 daemon 内部 `peers/snapshot.rs` 从 `CoreRuntime` 读取成员和在线状态。这样 WS 推送路径仍然绕过 `AppFacade`。

**处理结果**:
- `AppFacade` 增加 presence 事件订阅入口,对外返回 application 事件和订阅错误。
- `PresenceMonitor` 改为通过 `AppFacade` 订阅 presence 事件、读取 peer 快照。
- 删除 daemon 内部 `peers/snapshot.rs`,不再维护第二套 peer snapshot 规则。
- `PresenceMonitor` 的创建移动到 `DaemonApp::run`,复用同一个 `AppFacade`。

**边界判断**:
- daemon 仍负责把 application 事件转成 WS `peers.changed` 消息。
- daemon 装配阶段仍直接持有部分子 facade / runtime 来组装 `AppFacade`,归入 composition root 收口 todo。
...
### F-131 · peers 查询和 paired devices 可经 AppFacade 统一入口收口

**发现时间**:2026-04-26

**背景**:`DaemonQueryService::peers()` 原来直接持有 `CoreRuntime`、`PresencePort`,并调用 daemon 内部的 `derive_peer_snapshot`。这让 HTTP/WS 查询入口直接认识 core runtime 和 presence port,不符合“外部只走 `AppFacade`”的边界要求。

**处理结果**:
- `AppFacade` 增加 `list_peer_snapshots()` 和 `list_members()` 统一入口。
- application 层新增 `PeerSnapshotView`,对外提供稳定字符串和值,不暴露 core presence 类型。
- `MemberRosterFacade` 内部复用 roster + presence 聚合规则生成 peer 快照。
- `DaemonQueryService` 改为只持有 `AppFacade`,`/peers` 和 paired devices 查询都从 `AppFacade` 调用。
- `DaemonApp` 不再为 query service 持有和传入 `PresencePort`。

**边界判断**:
- `PresenceMonitor` 仍直接使用 `PresencePort` 和 `derive_peer_snapshot`,这是该 todo 的下一段工作。
- daemon 装配阶段仍接收 `MemberRosterFacade` 来组装 `AppFacade`,后续由 composition root 收口 todo 继续处理。
...
### F-118 · daemon 成员/配对入口已可通过 uc-application 应用模型收口

**发现时间**:2026-04-26

**背景**:daemon 的成员相关 HTTP API 之前直接使用 `uc_core::DeviceId`、`uc_core::MemberSyncPreferences`、`uc_application::membership::usecases::*`。这违反新的边界目标:外部入口只应面对 `uc-application` 的应用模型,不直接认识 core 领域对象或 use case。

**已收口范围**:
- `GET/PATCH /member/:device_id/sync-preferences`
- `POST /pairing/unpair`
- `paired_devices` 查询路径

**新的边界形态**:
- `uc-application::facade::MemberRosterFacade` 新增应用层方法:
  - `list_members()`
  - `get_sync_preferences(device_id: &str)`
  - `update_sync_preferences(device_id: &str, MemberSyncPreferencesPatch)`
  - `revoke_member(device_id: &str)`
- daemon 只做 HTTP DTO ↔ application DTO 投影,不再做 core 模型合并规则。
- 成员偏好 patch 合并规则从 daemon 移入 `uc-application/src/facade/roster/commands.rs`。
...
**剩余 daemon 泄漏点**:
- `entrypoint.rs` / `app.rs` 仍直接依赖 `uc-bootstrap`、`uc-app::CoreRuntime`、`uc-core::PresencePort`、`uc-platform::LocalClipboard`
- clipboard workers 仍直接使用 core clipboard snapshot / ids / platform watcher
- search/settings/encryption/lifecycle API 仍直接使用 `CoreUseCases` 和 core 模型
- `uc_application::space_access::SpaceAccessFacade` 仍是旧 facade 路径,不在统一 `src/facade` 下

**下一步建议**:优先收 `daemon api::settings` 或 `daemon api::search`。两者都是 HTTP DTO 直接映射 core 模型的典型问题;其中 settings 范围更大但边界更清晰。

### F-119 · daemon settings API 的边界问题集中在 patch 合并规则

**发现时间**:2026-04-26

**现状**:
- `uc-daemon/src/api/settings.rs` 直接构造 `uc_app::usecases::CoreUseCases`
- handler 直接持有 `uc_core::settings::model::Settings`
- `merge_settings_patch` 在 daemon 内把 HTTP patch DTO 合并进 core settings model
- retention rules / theme / sync frequency / shortcut key 等 DTO → core 转换散落在 daemon-contract 的 `From<Dto> for core::*`

**切片建议**:
1. 在 `uc-application/src/facade/settings/` 新增 `SettingsFacade`
2. facade 对外暴露 application 层 `SettingsView` / `SettingsPatch`
3. `SettingsFacade::update(patch)` 内部负责 load → merge → save → return view
4. daemon `api/settings.rs` 只做 `SettingsPatchDto -> SettingsPatch` 和 `SettingsView -> SettingsDto`

**注意**:
- `uc-daemon-contract` 当前仍直接依赖 `uc-core` 来实现 DTO 转换,本切片可以先不动,但后续若目标是“外部完全不知道 core”,contract 也要删除 `uc-core` 依赖。
- 这块范围大,但合并规则是纯内存转换,适合先用 `uc-application` 单测锁住行为再迁移 daemon。

**已执行结果**:
- 新增 `uc-application/src/facade/settings/` 承接 settings 读取、更新和 patch 合并。
- daemon `api/settings.rs` 已切换为 `SettingsFacade` 调用方。
- daemon 仍负责 HTTP DTO ↔ application model 投影,但不再使用 core settings model 或 `CoreUseCases`。
- 验证:`cargo test -p uc-application facade::settings --lib`、`cargo check -p uc-daemon` 均通过。

### F-120 · daemon device/me 可独立收口为 device facade

**发现时间**:2026-04-26

**现状**:`GET /device/me` 只需要本机 device id 和设置中的 device name。旧实现通过 daemon → `uc-app::CoreUseCases` → `GetLocalDeviceInfo` 取得,导致 daemon 为一个只读查询继续依赖 `uc-app` pairing usecase。

**处理**:
- 新增 `uc-application::facade::device::DeviceFacade`
- 将 device name 修剪和 fallback 规则迁到 application facade
- daemon `api/device.rs` 改为调用 `DeviceFacade::local_device_info`
- 删除 daemon conversion 中 `uc_app::usecases::pairing::LocalDeviceInfo` 的投影
...
## F-140 — uc-tauri AppUseCases / runtime.usecases() / apply_autostart 全是 dead code

**事实**:
对整个 src-tauri/ 树全文搜索 `\.usecases()` 和 `apply_autostart()`,过滤掉文档注释后:
- `\.usecases()` 实际调用次数:0
- `apply_autostart()` 实际调用次数:0
- `AppUseCases` 仅在 `bootstrap/runtime.rs` 内部定义和 `bootstrap/mod.rs` 的 re-export 中出现

**含义**:
`AppUseCases<'a>` 这一整个"包装 CoreUseCases + apply_autostart"的类型是 dead code。`AppRuntime::usecases()` 也没人调。这意味着 D14 在拆 CoreUseCases 这件事上完全无业务影响 —— 可以直接删整个 AppUseCases 类型,无需替换。

**为什么会这样**:`commands/autostart.rs` 直接调 `tauri_plugin_autostart::ManagerExt::autolaunch()` 完成开机自启,从未走 `ApplyAutostartSetting` use case。其他业务命令在 D11/D12 期间(或更早)都迁到了 daemon HTTP API,Tauri 端只剩 OS 层壳命令(tray、autostart、updater、storage 打开目录、quick_panel)和工具命令(get_pid)。

**用法建议**:D14 直接删 `AppUseCases` 整个类型 + `usecases()` 方法 + `apply_autostart()` 方法 + `bootstrap/mod.rs` re-export,无替代。

---

## F-141 — Tauri autostart commands 不走 use case,直接调插件 trait

**事实**:
`commands/autostart.rs::enable_autostart / disable_autostart / is_autostart_enabled` 三个命令实现:
```rust
let autostart_manager = app_handle.autolaunch();
autostart_manager.enable()?;
```
完全用 `tauri_plugin_autostart::ManagerExt` 的 `.autolaunch()` extension method,**没有路过 `ApplyAutostartSetting` use case**。

**含义**:
`uc_platform::usecases::ApplyAutostartSetting` 这个类型在 src-tauri 内**至少在 commands 层无 caller**(`apply_autostart()` 也无 caller)。它可能在 daemon 端的 settings sync flow 里有用,但与 uc-tauri 的 D14 改造无关。

**用法建议**:删除 `AppUseCases::apply_autostart` 不会影响任何现有调用,自然不需要补 facade 替代。

---

## F-142 — AppRuntime 真正活跃的方法清单(D14 改造范围)

**事实**:
`grep -rn` 整个 src-tauri 后,`AppRuntime` 实际被调用的方法有:
- `task_registry()` — main.rs L88/178/276,`wiring.rs` 接受 `&Arc<TaskRegistry>` 参数
- `set_app_handle()` — main.rs L158
- `app_handle()` — `AppUseCases::apply_autostart` 内部用(但 apply_autostart 是 dead code,所以可去除)
- `app_handle_cell()` — 0 caller(可删)
- `event_emitter()` — 待确认,可能用于 daemon 启动后 swap
- `set_event_emitter()` — 同上
- `is_encryption_ready()` — 0 caller(可删)
- `has_completed_setup()` — 0 caller(可删)
- `settings_port()` — main.rs L203/237/290/310
- `wiring_deps()` — main.rs L274 一次(`runtime.wiring_deps().settings.clone()`)
- `clipboard_integration_mode()` — 0 caller(可删)
- `device_id()` — `commands/mod.rs:26` 一次
- `core()` — `commands/storage.rs:26` 一次(`runtime.core().storage_paths()`)
- `usecases()` — 0 caller(可删,见 F-140)

**含义**:D14 收缩后的 AppRuntime API 表面会非常小:
- 业务入口:`app_facade() -> &Arc<AppFacade>`(新增,给未来命令用)
- 字段访问:`task_registry() / settings_port() / storage_paths() / device_id()`
