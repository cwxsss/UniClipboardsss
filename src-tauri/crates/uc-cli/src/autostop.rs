...
mod tests {
    ...
    fn session(spawned: bool) -> LocalDaemonSession {
        LocalDaemonSession {
    ...
    #[test]
    fn arm_spawned_sets_pid_sentinel() {
        let guard = AutostopGuard::arm(&session(true));
        assert!(guard.pid.is_some());
        // Avoid running Drop (would try to read real PID file); defuse.
        std::mem::forget(guard);
    }

    #[test]
    fn arm_unspawned_is_noop() {
        let guard = AutostopGuard::arm(&session(false));
        assert!(guard.pid.is_none());
        // Drop is safe: it's a noop.
        drop(guard);
    }

    #[test]
    fn disarm_clears_pid() {
        let mut guard = AutostopGuard::arm(&session(true));
        guard.disarm();
        assert!(guard.pid.is_none());
        drop(guard);
    }
