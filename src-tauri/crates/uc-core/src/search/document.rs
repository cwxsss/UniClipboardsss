...
mod tests {
    ...
    /// Compile-time invariant: this file must NOT contain the soft-delete field name.
    /// Arch spec question 4 resolved hard-delete as the semantic — the field is absent by design.
    #[test]
    fn search_document_has_no_deleted_at_field() {
        let src = include_str!("document.rs");
        // The forbidden field name is split to avoid the grep itself triggering the assertion.
        let forbidden = ["deleted", "_at_ms"].concat();
        assert!(
            !src.contains(&forbidden),
            "SearchDocument must not have the soft-delete timestamp field — hard-delete semantic"
        );
    }
