...
pub async fn wait_for_encryption_session_ready(
    ...
    let client = match DaemonQueryClient::new(connection_state) {
        Ok(client) => client,
        Err(error) => {
            tracing::warn!(error = %error, error_kind = "daemon_client_build_failed", retryable = false, "failed to build local daemon encryption-state client");
            return false;
        }
    };
...
pub async fn run_cold_launch_actions(
    ...
    let settings_client = match DaemonSettingsClient::new(connection_state.clone()) {
        Ok(client) => client,
        Err(error) => {
            tracing::warn!(error = %error, error_kind = "daemon_client_build_failed", retryable = false, "failed to build local daemon settings client");
            return None;
        }
    };
...
async fn restore_last_entry(connection_state: DaemonConnectionState) {
    let search_client = match DaemonSearchClient::new(connection_state.clone()) {
        Ok(client) => client,
        Err(error) => {
            tracing::warn!(error = %error, error_kind = "daemon_client_build_failed", retryable = false, "failed to build local daemon search client");
            return;
        }
    };
    let clipboard_client = match DaemonClipboardClient::new(connection_state) {
        Ok(client) => client,
        Err(error) => {
            tracing::warn!(error = %error, error_kind = "daemon_client_build_failed", retryable = false, "failed to build local daemon clipboard client");
            return;
        }
    };
