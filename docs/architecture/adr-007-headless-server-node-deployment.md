...
- 通过 `uniclip mobile-sync lan enable` / `devices add` 非交互启用并铸设备凭据。
...
- 手机安装 URL/QR 需指向 Caddy 的 `https://域名`，而现有构造是 `http://<advertise-ip>:<lan_port>`。因此 **`mobile-sync lan enable` 扩展 `--advertise-url <完整 base URL>`**，URL/QR 构造支持 scheme/host/port 覆盖；保留 `--advertise <ip>` 的 LAN 形态。
...
docker compose run --rm app uniclip mobile-sync lan enable --advertise-url https://<域名> --accept-network-risk
docker compose run --rm app uniclip mobile-sync devices add --label "<设备>"
...
- 手机网关、入站落库、headless 解密 **复用既有能力**，新代码集中在：`ServerHeadless` 变体、Noop/跳过 watcher 装配、`run_standalone_from_env`、iroh `bind_port`/`public_addr`、`--advertise-url`、Docker/Caddy 编排。
