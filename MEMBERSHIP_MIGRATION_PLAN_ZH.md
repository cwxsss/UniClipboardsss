...
**下一步**：阶段 0.4（方案 B2），按 §5.4 "阶段 0.4 commit 拆分" 的 0.4.1 → 0.4.4 顺序推进：pairing 协议层 6 个文件平移到 `uc-application::pairing`（行为不变）→ 写入切 `TrustPeerOrchestrator` → 删 staged store / facade → daemon 切 uc-application。完成后 0.5 删除 `PairedDevice` 族 Rust 类型，再切入阶段 A（space_access 搬家 + `SpaceAccessContext` 扩字段 + admit 挂接）。
...
| 0.4 | ⏳ 下一步 | **pairing 协议层整体从 `uc-app/pairing/` 平移到 `uc-application/pairing/`**（B2 方案，2026-04-17 决策 D26），同时切换写入目标到 `TrustedPeer`；`staged_paired_device_store` / `uc-app/pairing/facade.rs` 删除；daemon 直切 `uc-application::pairing`；旧错误类型 `FailureReason` / `PairingBusy` 翻译到 `TrustStateEvent` 三档 | 涉及 6 个协议文件搬家 + bootstrap / daemon import 改名 + 写入路径切换；非协议类 use case（`list_paired_devices` / `unpair_device` / 等 8 个）留在 uc-app，到 Phase 3 再切换 |
...
| **0.4.2** | 写入路径切换到 `TrustPeerOrchestrator` | `uc-application/pairing/protocol_handler.rs`：`PairingAction::PersistPairedDevice` 分支替换为 `trust_peer_orchestrator.record_session_opened(...)` + `.confirm_verification()`；删除 `device_repo.upsert` / `dual_write_member` / `space_member_from_paired_device` helper；删除 `FailureReason` / `PairingBusy` 错误类型，翻译到 `TrustStateEvent::{TimedOut, ProtocolError, UserCancelled}` 三档；bootstrap 构造 `TrustPeerOrchestrator` 单例（D19）并注入 pairing orchestrator；摘掉 `DevicePorts::member_repo` 在 pairing 装配链的注入路径（`space_member` 写入由阶段 A 的 admit 承担）；重写本文件 4 个 Phase 2 dual_write 单元测试为 trusted_peer 断言 |
| **0.4.3** | 切断 space_access 的反向依赖 + 删 staged store / facade | `uc-app/space_access/persistence_adapter.rs` 改用 `TrustedPeerRepositoryPort::get(...).is_some()` 替代 `staged_paired_device_store` 查询；删除 `uc-app/pairing/staged_paired_device_store.rs`；删除 `uc-app/pairing/facade.rs`（D25 — 当前无调用方）；`uc-app/pairing/mod.rs` 移除对 staged store / facade 的 re-export；bootstrap（`builders.rs` / `assembly.rs`）摘掉 staged store 构造与字段 |
| **0.4.4** | daemon UI 触发路径切到 `uc-application` UseCase | `uc-daemon` 的 pairing 用户触发命令（confirm / cancel）从"直接驱动 orchestrator"改为调 `ConfirmPeerVerificationUseCase` / `CancelTrustingUseCase`（D22 thin wrapper）；`FailureReason` 的模式匹配替换为 `TrustedPeerApplicationError` / `TrustAbortReason`；daemon 集成测试跑绿 |
