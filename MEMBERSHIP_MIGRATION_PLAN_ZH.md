...
**下一步**：阶段 0.4（方案 B2），按 §5.4 "阶段 0.4 commit 拆分" 的 0.4.1 → 0.4.4 顺序推进。**0.4.1 + 0.4.2 已完成**（写入路径从 PairedDevice 切到 TrustedPeer；`FailureReason` 留到 0.4.2.b）。当前进入 **0.4.3**：`uc-app/space_access/persistence_adapter.rs` 反向依赖切断 + 删 staged store / `uc-app/pairing/facade.rs`。完成后继续 0.4.4（daemon 切 UseCase）、0.4.2.b（`FailureReason` 收口）、0.5（删除 `PairedDevice` 族 Rust 类型），再切入阶段 A（space_access 搬家 + `SpaceAccessContext` 扩字段 + admit 挂接）。
