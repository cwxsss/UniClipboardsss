...
> 状态：**Phase 1 / Phase 2 / 阶段 0（0.0–0.5）/ 阶段 A（A.1–A.4）/ 阶段 B（B.0–B.6）/ 阶段 C（C.1–C.5）/ Phase 3 全部（3.1 / 3.2 / 3.3 / 3.4 · scope C / 3.5 / 3.6 · scope A）/ Phase 4b PR-1 / PR-2 / PR-3 / PR-4 均已提交；下一步 PR-5（DROP TABLE + 删 `PairedDeviceRepositoryPort` / `PairedDevice` / `PairingState` / uc-infra 残留类型）。**
...
| — | `PairedDeviceRepositoryPort` trait / `DieselPairedDeviceRepository` 实现归并入 PR-5（与 `DROP TABLE paired_device` 同批清理，避免跨 PR 残留孤立 trait） | ⏳ |
| PR-5 | `DROP TABLE paired_device` migration + 删 uc-core/uc-infra 残留类型（原 Phase 5 合并进来） | ⏳ |
