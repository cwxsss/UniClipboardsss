...
```bash
INIT=$(node "/Volumes/ExternalSSD/myprojects/uniclipboard-desktop/.claude/get-shit-done/bin/gsd-tools.cjs" init phase-op "${after_phase}")
...
```bash
RESULT=$(node "/Volumes/ExternalSSD/myprojects/uniclipboard-desktop/.claude/get-shit-done/bin/gsd-tools.cjs" phase insert "${after_phase}" "${description}")
```
