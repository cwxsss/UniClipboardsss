...
- [ ] **T02: Migrate remaining clipboard UI and preview consumers to daemon transport** — Replace the live clipboard business callers that still import runtime functions from `@/api/clipboardItems`. Move list hydration, single-entry reloads, preview/detail/resource loading, and clear/stats consumers onto daemon clients/store thunks, while leaving `clipboardItems.ts` as a types-and-native-utility module only.
  - Estimate: 1.5h
