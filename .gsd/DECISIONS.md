...
| D009 | M003-fbgash / S07 / T01 (2026-03-30) | pattern | Daemon WebSocket auth normalization: extract_session_token() helper with dual-transport validation | One extract_session_token() helper normalizes Authorization header AND ?auth= query param into the same JWT/PID/rate-limit validation pipeline | Browsers cannot send arbitrary headers on WebSocket upgrades — only URL query parameters are safe for browser-based clients. Rather than adding a separate unauthenticated query-param bypass, extract_session_token() tries Authorization header first, then ?auth= query param, and feeds both to the same validation path. This preserves all existing JWT verification, PID whitelist, and rate limiting checks with zero security weakening. The Rust tests prove both paths yield identical security outcomes. | Yes — if browser adds support for safe WebSocket auth headers, or if a separate unauthenticated path is explicitly desired. | agent |
